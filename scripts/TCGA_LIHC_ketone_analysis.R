...
# Associated with: "Ketone body metabolism across the MASLD-to-HCC continuum:
# cell-type-specific mechanisms, evidence gaps, and a translational roadmap"
#
# Changes from v1:
...
#
# Interpretation note:
#   TCGA-LIHC is used only as mixed-etiology supportive / broad HCC
#   exploratory evidence. These analyses should not be interpreted as
#   MASLD-HCC-specific proof or as clinical recommendations.
#
# Data sources:
...
file_fpkm     <- file.path(dir_xena, "TCGA-LIHC.star_fpkm.tsv.gz")
file_survival <- file.path(dir_xena, "TCGA-LIHC.survival.tsv.gz")
file_clinical <- file.path(dir_xena, "TCGA-LIHC.clinical.tsv.gz")

stopifnot(file.exists(file_fpkm))
stopifnot(file.exists(file_survival))
stopifnot(file.exists(file_clinical))

# Optional sections:
# Sections 7-9 generate Fig. 5E-G and require ESTIMATE-compatible gene-symbol input.
# Keep FALSE when reproducing Fig. 5H and Supplementary Fig. S1 only.
...
fpkm <- read.table(file_fpkm,
                   header = TRUE, sep = "\t", row.names = 1,
                   check.names = FALSE)
cat("Dimensions:", nrow(fpkm), "genes x", ncol(fpkm), "samples\n")

# Load survival and clinical data
surv_raw <- read.table(file_survival,
                       header = TRUE, sep = "\t")
clin     <- read.table(file_clinical,
...
GDCdownload(query_maf, directory = dir_gdc)
maf <- GDCprepare(query_maf, directory = dir_gdc)
...
ggsave(file.path(dir_output, "ketone_genes_expression_v2.png"),
...
png(file.path(dir_output, "survival_OXCT1.png"), width = 8, height = 6, units = "in", res = 300)
...
png(file.path(dir_output, "survival_HMGCS2.png"), width = 8, height = 6, units = "in", res = 300)
...
png(file.path(dir_output, "survival_combined.png"), width = 10, height = 7, units = "in", res = 300)
...
if (run_sections_7_to_9) {  # optional: regenerate Sections 7-9 / Fig. 5E-G
# -----------------------------------------------------------------------------
...
write.table(fpkm[, grepl("-01", colnames(fpkm))],
            file.path(dir_output, "expr_for_estimate.txt"), sep = "\t", quote = FALSE)
filterCommonGenes(file.path(dir_output, "expr_for_estimate.txt"),
                  file.path(dir_output, "expr_filtered.gct"), id = "GeneSymbol")
estimateScore(file.path(dir_output, "expr_filtered.gct"),
              file.path(dir_output, "estimate_scores.gct"), platform = "illumina")

scores_raw <- read.table(file.path(dir_output, "estimate_scores.gct"),
...
ggsave(file.path(dir_output, "OXCT1_ImmuneScore_correlation.png"),
...
ggsave(file.path(dir_output, "OXCT1_exhaustion_markers.png"),
...
ggsave(file.path(dir_output, "HMGCS2_OXCT1_correlation.png"),
       plot = p_axis, width = 6, height = 5, dpi = 300)


}  # end optional Sections 7-9
# -----------------------------------------------------------------------------
...
p_ctnnb1 <- ggplot(merged, aes(x = CTNNB1_mut, y = OXCT1, fill = CTNNB1_mut)) +
  ...
  labs(title = "OXCT1 expression by CTNNB1 status",
       subtitle = "Wilcoxon p = 0.016; mixed-etiology exploratory analysis",
...
ggsave(file.path(dir_pdf, "Fig5H_CTNNB1_OXCT1_vector.pdf"),
       plot = p_ctnnb1, width = 7, height = 5.5, dpi = 300)
...
ggsave(file.path(dir_pdf, "SuppFig_MI_sensitivity_vector.pdf"),
